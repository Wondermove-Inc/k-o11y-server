...
function useTimezoneFormatter({
	...
	const formatTimezoneAdjustedTimestamp = useCallback(
		(
			...
			const timestamp = dayjs.utc(input).valueOf();
			...
			// Format timestamp (parse as UTC, then convert to target timezone)
			const formattedValue = dayjs.utc(input).tz(userTimezone.value).format(format);
