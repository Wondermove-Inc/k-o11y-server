...
export function DashboardProvider({
	...
	const dashboardResponse = useQuery(
		[REACT_QUERY_KEY.DASHBOARD_BY_ID, isDashboardPage?.params, dashboardId, staticDashboard?.id],
		{
			enabled: !staticDashboard && (!!isDashboardPage || !!isDashboardWidgetPage) && isLoggedIn,
			initialData: staticDashboard ? {
				httpStatusCode: 200,
				data: staticDashboard,
			} as SuccessResponseV2<Dashboard> : undefined,
			queryFn: async () => {
	...
	useEffect(() => {
		if (staticDashboard && !selectedDashboard) {
			// Transform dashboard variables the same way as regular dashboards
			...
			// Initialize variables for static dashboard
			const variables = updatedDashboardData.data.variables;
			if (variables) {
				initializeDefaultVariables(variables, getUrlVariables, updateUrlVariable);
			}
		}
	}, [staticDashboard, selectedDashboard, getUrlVariables, updateUrlVariable]);
	...
	const value: IDashboardContext = useMemo(
		() => ({
			...
			columnWidths,
			setColumnWidths,
			isStaticDashboard: !!staticDashboard,
		}),
		// eslint-disable-next-line react-hooks/exhaustive-deps
		[
			isDashboardSliderOpen,
