...
interface TraceDetailsV2Props {
	embeddedMode?: boolean;
}

function TraceDetailsV2(props: TraceDetailsV2Props): JSX.Element {
	const { embeddedMode = false } = props;
	...
	const handleEmbeddedBack = useCallback((): void => {
		if (!embeddedMode) {
			return;
		}
		...
		window.parent?.postMessage(
			{
				type: 'KO11Y_TRACE_EMBED_CLOSE',
				traceId,
			},
			window.location.origin,
		);
	}, [embeddedMode, traceId]);
		...
		>
				<ResizablePanel minSize={20} maxSize={80} className="trace-left-content">
					...
					rootSpanName={traceData?.payload?.rootServiceEntryPoint || ''}
						...
						notFound={noData}
						onBackClick={embeddedMode ? handleEmbeddedBack : undefined}
					/>
				{!noData ? (
