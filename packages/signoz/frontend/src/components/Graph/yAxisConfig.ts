...
export const getYAxisFormattedValue = (
	...
	try {
		...
		// For 'short' format with small numbers, show as plain numbers for better readability
		if (format === 'short' && Math.abs(numValue) < 1000) {
			if (precision === 0 || Number.isInteger(numValue)) {
