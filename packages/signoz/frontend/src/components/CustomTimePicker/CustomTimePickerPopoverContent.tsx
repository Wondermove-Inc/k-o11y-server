...
function CustomTimePickerPopoverContent({
	...
	// ServiceMap만 기존 UI 사용, 나머지는 모두 TimeRangeSelector 사용
	const isServiceMapPage = useMemo(
		() => pathname.startsWith(ROUTES.SERVICE_MAP_BASE),
		[pathname],
	);
	...
	// ServiceMap은 기존 UI 사용, 나머지 모든 페이지는 TimeRangeSelector 사용
	if (!isServiceMapPage) {
		return (
			<TimeRangeSelector
				options={options}
