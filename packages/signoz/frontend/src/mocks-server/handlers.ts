...
export const handlers = [
	...
	rest.get(
		'http://localhost/api/v1/org/preferences/alert_base_url',
		(_, res, ctx) =>
			res(
				ctx.status(200),
				ctx.json({
					...
					data: {
						name: 'alert_base_url',
						description: 'Base URL used for alert links in notifications.',
						valueType: 'string',
	...
	rest.get('http://localhost/api/v1/alertmanager/settings', (_, res, ctx) =>
		res(
			ctx.status(200),
			ctx.json({
				status: 'success',
	...
	rest.put('http://localhost/api/v1/alertmanager/settings', (_, res, ctx) =>
		res(
			ctx.status(200),
			ctx.json({
				status: 'success',
