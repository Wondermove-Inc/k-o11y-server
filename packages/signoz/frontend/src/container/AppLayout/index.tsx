...
import useKO11yAuth from 'hooks/useKO11yAuth';
...
function AppLayout(props: AppLayoutProps): JSX.Element {
	...
	// K-O11y SSO: listen for IPC auth events from Freelens host
	useKO11yAuth();
	...
	const isTraceEmbedRoute = useMemo(
		() =>
			Boolean(
				matchPath(pathname, {
					path: '/trace-embed/:id',
		...
		<div className="workspace-restricted-banner">
			{activeLicense?.state === LicenseState.TERMINATED && (
				<>
					...
					<a href="mailto:support@example.com">support@example.com</a> for new license
				</>
			)}
			...
			{activeLicense?.state === LicenseState.CANCELLED && (
				<>
					...
					<a href="mailto:support@example.com">support@example.com</a> for reactivation
			...
			{activeLicense?.state === LicenseState.EVALUATION_EXPIRED && (
				<>
					...
					<a href="mailto:support@example.com">support@example.com</a> for next steps to
					avoid termination of license as per our{' '}
					<a
						href="https://signoz.io/terms-of-service"
						target="_blank"
	...
	const getSidebarStateFromLocalStorage = useCallback((): boolean => {
		try {
			...
			// If no value is stored, default to true (sidebar open)
			if (storedValue === null || storedValue === undefined) {
				return true;
			}
			return storedValue === 'true';
		} catch {
			return true; // Default to sidebar open on error
		}
	}, []);
