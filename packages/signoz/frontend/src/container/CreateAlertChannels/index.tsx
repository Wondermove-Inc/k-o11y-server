...
const slackTitleTemplate = [
	'{{- $severity := .CommonLabels.severity -}}',
	'{{- if eq $severity "critical" -}}[CRITICAL] Alert Triggered{{- else if eq $severity "error" -}}[ERROR] Alert Triggered{{- else if eq $severity "warning" -}}[WARNING] Alert Triggered{{- else -}}[INFO] Alert Triggered{{- end }}',
].join('\n');

const slackColorTemplate = [
	'{{- $severity := .CommonLabels.severity -}}',
	'{{- if eq $severity "critical" -}}#E01E5A{{- else if eq $severity "error" -}}#E01E5A{{- else if eq $severity "warning" -}}#ECB22E{{- else -}}#125AED{{- end }}',
].join('');

const slackTextTemplate = [
	'{{ range .Alerts -}}',
...
function CreateAlertChannels({
	...
	const [selectedConfig, setSelectedConfig] = useState<
		...
		>({
			send_resolved: true,
			text: slackTextTemplate,
			title: slackTitleTemplate,
			color: slackColorTemplate,
		});
