...
function SideNav({ isPinned }: { isPinned: boolean }): JSX.Element {
	...
	useEffect(() => {
		...
		// Always start fresh from default items
		let items: (SidebarItem | DropdownSeparator)[] = [...DefaultHelpSupportDropdownMenuItems];

		// Remove invite-collaborators if not admin
		if (!isAdmin) {
			items = items.filter(
		...
		// Remove chat-support if conditions not met
		if (
			!(
				isLoggedIn &&
			...
			items = items.filter((item) => !('key' in item) || item.key !== 'chat-support');
		}

		setHelpSupportDropdownMenuItems(items);
	}, [
		isAdmin,
		isLoggedIn,
		isChatSupportEnabled,
