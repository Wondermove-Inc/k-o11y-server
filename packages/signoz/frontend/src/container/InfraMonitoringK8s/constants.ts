...
export const underscoreMap = {
	...
	[K8sCategory.ROLLOUTS]: 'k8s_pod_cpu_usage',
};
...
export const dotMap = {
	...
	[K8sCategory.ROLLOUTS]: 'k8s.pod.cpu.usage',
};
...
export function GetPodsQuickFiltersConfig(
	...
	const deploymentKey = dotMetricsEnabled
		? 'k8s.deployment.name'
		: 'k8s_deployment_name';
	const rolloutKey = dotMetricsEnabled
		? 'k8s.rollout.name'
		: 'k8s_rollout_name';
	...
	return [
		...
		},
		{
			...
			attributeKey: {
				...
				dataType: DataTypes.String,
				type: 'resource',
				id: `${rolloutKey}--string--resource--false`,
			},
			aggregateOperator: 'noop',
			aggregateAttribute: cpuUtilizationMetric,
			dataSource: DataSource.METRICS,
...
export function GetRolloutsQuickFiltersConfig(
	dotMetricsEnabled: boolean,
): IQuickFiltersConfig[] {
	const rolloutKey = dotMetricsEnabled
		? 'k8s.rollout.name'
