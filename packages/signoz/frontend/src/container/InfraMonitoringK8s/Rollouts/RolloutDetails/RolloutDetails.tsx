...
import RolloutEvents from '../../EntityDetailsUtils/EntityEvents';
import RolloutLogs from '../../EntityDetailsUtils/EntityLogs';
import RolloutMetrics from '../../EntityDetailsUtils/EntityMetrics';
import RolloutTraces from '../../EntityDetailsUtils/EntityTraces';
import {
	getRolloutMetricsQueryPayload,
	rolloutWidgetInfo,
} from './constants';
import { RolloutDetailsProps } from './RolloutDetails.interfaces';

function RolloutDetails({
	rollout,
	onClose,
	isModalTimeSelection,
}: RolloutDetailsProps): JSX.Element {
			...
			items: [
				{
					id: uuidv4(),
					key: {
						key: QUERY_KEYS.K8S_ROLLOUT_NAME,
						dataType: DataTypes.String,
						type: 'resource',
						id: 'k8s_rollout_name--string--resource--false',
					},
					op: '=',
					value: rollout?.meta.k8s_rollout_name || '',
				},
				{
					id: uuidv4(),
					key: {
					...
					op: '=',
					value: rollout?.meta.k8s_namespace_name || '',
				},
			],
		};
	}, [
		rollout?.meta.k8s_rollout_name,
		rollout?.meta.k8s_namespace_name,
		searchParams,
	]);
	...
	}, [rollout?.meta.k8s_rollout_name, searchParams]);
		...
		(value: IBuilderQuery['filters'], view: VIEWS) => {
			setLogAndTracesFilters((prevFilters) => {
				const primaryFilters = prevFilters?.items?.filter((item) =>
					[QUERY_KEYS.K8S_ROLLOUT_NAME, QUERY_KEYS.K8S_NAMESPACE_NAME].includes(
						item.key?.key ?? '',
					),
				);
				...
				const newFilters = value?.items?.filter(
					(item) =>
						item.key?.key !== 'id' &&
						item.key?.key !== QUERY_KEYS.K8S_ROLLOUT_NAME,
				);
				...
				const updatedFilters = {
					...
					items: filterDuplicateFilters(
						[
							...(primaryFilters || []),
							...(value?.items?.filter(
								(item) => item.key?.key !== QUERY_KEYS.K8S_ROLLOUT_NAME,
							) || []),
						].filter((item): item is TagFilterItem => item !== undefined),
					),
				};
	...
	const handleChangeEventsFilters = useCallback(
		(value: IBuilderQuery['filters'], view: VIEWS) => {
			setEventsFilters((prevFilters) => {
				const rolloutKindFilter = prevFilters?.items?.find(
					(item) => item.key?.key === QUERY_KEYS.K8S_OBJECT_KIND,
				);
				const rolloutNameFilter = prevFilters?.items?.find(
			...
			{rollout && (
				<>
					<div className="entity-detail-drawer__entity">
						<div className="entity-details-grid">
							...
							<div className="values-row">
								...
								<Typography.Text className="entity-details-metadata-value">
									<Tooltip title={rollout.meta.k8s_cluster_name}>
										{rollout.meta.k8s_cluster_name}
									</Tooltip>
								</Typography.Text>
								<Typography.Text className="entity-details-metadata-value">
									<Tooltip title={rollout.meta.k8s_namespace_name}>
										{rollout.meta.k8s_namespace_name}
									</Tooltip>
								</Typography.Text>
							</div>
					...
					{selectedView === VIEW_TYPES.METRICS && (
						<RolloutMetrics<K8sRolloutsData>
							timeRange={modalTimeRange}
							isModalTimeSelection={isModalTimeSelection}
							handleTimeChange={handleTimeChange}
							selectedInterval={selectedInterval}
							entity={rollout}
							entityWidgetInfo={rolloutWidgetInfo}
							getEntityQueryPayload={getRolloutMetricsQueryPayload}
							category={K8sCategory.ROLLOUTS}
							queryKey="rolloutMetrics"
						/>
					)}
					{selectedView === VIEW_TYPES.LOGS && (
						<RolloutLogs
							...
							selectedInterval={selectedInterval}
							queryKeyFilters={[
								QUERY_KEYS.K8S_ROLLOUT_NAME,
								QUERY_KEYS.K8S_NAMESPACE_NAME,
							]}
							queryKey="rolloutLogs"
							category={K8sCategory.ROLLOUTS}
						/>
					)}
					{selectedView === VIEW_TYPES.TRACES && (
