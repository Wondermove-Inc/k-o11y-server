...
func TestDispatcherGetOrCreateRouteUsesRootRouteOptions(t *testing.T) {
	root := &dispatch.Route{
	...
	route := dispatcher.getOrCreateRoute("receiver-a")

	require.Equal(t, root.RouteOpts.GroupWait, route.RouteOpts.GroupWait)
	require.Equal(t, root.RouteOpts.GroupInterval, route.RouteOpts.GroupInterval)
	require.Equal(t, root.RouteOpts.RepeatInterval, route.RouteOpts.RepeatInterval)
}
