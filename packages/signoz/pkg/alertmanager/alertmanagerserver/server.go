...
func (server *Server) UpdateExternalURL(externalURL *url.URL) bool {
	if externalURL == nil {
		return false
	}

	if server.srvConfig.ExternalURL != nil && server.srvConfig.ExternalURL.String() == externalURL.String() {
		return false
	}

	server.srvConfig.ExternalURL = externalURL
	return true
}

func (server *Server) ReloadConfig(ctx context.Context) error {
	if server.alertmanagerConfig == nil {
		return nil
	}

	return server.SetConfig(ctx, server.alertmanagerConfig)
}
