...
func (api *API) GetSettings(rw http.ResponseWriter, req *http.Request) {
	...
	settings, err := api.alertmanager.GetSettings(ctx, claims.OrgID)
	if err != nil {
		render.Error(rw, err)
		return
	}

	render.Success(rw, http.StatusOK, settings)
}

func (api *API) UpdateSettings(rw http.ResponseWriter, req *http.Request) {
	ctx, cancel := context.WithTimeout(req.Context(), 10*time.Second)
	defer cancel()
	...
	update := new(alertmanagertypes.AlertmanagerSettingsUpdate)
	...
	settings, err := api.alertmanager.UpdateSettings(ctx, claims.OrgID, *update)
	if err != nil {
		render.Error(rw, err)
		return
	}
