...
func New(ctx context.Context, providerSettings factory.ProviderSettings, config alertmanager.Config, sqlstore sqlstore.SQLStore, orgGetter organization.Getter, notificationManager nfmanager.NotificationManager) (*provider, error) {
	...
	settingsStore := sqlalertmanagerstore.NewSettingsStore(sqlstore)
	preferenceModule := implpreference.NewModule(implpreference.NewStore(sqlstore), preferencetypes.NewAvailablePreference())
...
func (provider *provider) GetSettings(ctx context.Context, orgID string) (*alertmanagertypes.AlertmanagerSettingsResponse, error) {
	return provider.service.GetSettings(ctx, orgID)
}

func (provider *provider) UpdateSettings(ctx context.Context, orgID string, update alertmanagertypes.AlertmanagerSettingsUpdate) (*alertmanagertypes.AlertmanagerSettingsResponse, error) {
	return provider.service.UpdateSettings(ctx, orgID, update)
}
