...
func (m *defaultFieldMapper) FieldFor(
	...
	case schema.JSONColumnType{}:
		...
		if key.Materialized {
			materializedColName := telemetrytypes.FieldKeyToMaterializedColumnName(key)
			materializedColNameExists := telemetrytypes.FieldKeyToMaterializedColumnNameForExists(key)
			return fmt.Sprintf("multiIf(%s==true, %s, mapContains(%s, '%s'), %s, NULL)", materializedColNameExists, materializedColName, oldColumn.Name, key.Name, oldKeyName), nil
		} else {
			return fmt.Sprintf("multiIf(mapContains(%s, '%s'), %s, NULL)", oldColumn.Name, key.Name, oldKeyName), nil
		}
