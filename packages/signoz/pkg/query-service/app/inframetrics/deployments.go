...
func (d *DeploymentsRepo) getMetadataAttributes(ctx context.Context, req model.DeploymentListRequest) (map[string]map[string]string, error) {
	...
	for _, row := range attrsListResponse {
		...
		metaKey := workloadMetaKey(deploymentName, namespaceName, clusterName)
		if _, ok := deploymentAttrs[metaKey]; !ok {
			deploymentAttrs[metaKey] = map[string]string{}
		}

		for _, key := range req.GroupBy {
			deploymentAttrs[metaKey][key.Key] = stringData[key.Key]
		}
	}
...
func (d *DeploymentsRepo) GetDeploymentList(ctx context.Context, orgID valuer.UUID, req model.DeploymentListRequest) (model.DeploymentListResponse, error) {
	...
	var deploymentAttrs map[string]map[string]string
	var topDeploymentGroups, allDeploymentGroups []map[string]string
	var metaErr, topErr error

	done := make(chan struct{})
	go func() {
		deploymentAttrs, metaErr = d.getMetadataAttributes(ctx, req)
		done <- struct{}{}
	}()
	topDeploymentGroups, allDeploymentGroups, topErr = d.getTopDeploymentGroups(ctx, orgID, req, query)
	<-done
	...
	for _, result := range formattedResponse {
		for _, row := range result.Table.Rows {
			...
			metaKey := workloadMetaKey(record.DeploymentName, namespaceName, clusterName)
			if _, ok := deploymentAttrs[metaKey]; ok && record.DeploymentName != "" {
				record.Meta = deploymentAttrs[metaKey]
			}
	...
	if len(topDeploymentGroups) > 0 {
		groupByKeys := make([]string, len(req.GroupBy))
		for i, gb := range req.GroupBy {
			groupByKeys[i] = gb.Key
		}
		validGroups := make(map[string]bool, len(topDeploymentGroups))
		for _, g := range topDeploymentGroups {
			validGroups[buildGroupTupleKey(g, groupByKeys)] = true
		}
		filteredRecords := make([]model.DeploymentListRecord, 0, len(topDeploymentGroups))
