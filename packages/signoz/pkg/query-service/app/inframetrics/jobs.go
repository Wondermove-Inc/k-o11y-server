...
func (d *JobsRepo) getMetadataAttributes(ctx context.Context, req model.JobListRequest) (map[string]map[string]string, error) {
	...
	for _, row := range attrsListResponse {
		...
		metaKey := workloadMetaKey(jobName, namespaceName, clusterName)
		if _, ok := jobAttrs[metaKey]; !ok {
			jobAttrs[metaKey] = map[string]string{}
		}

		for _, key := range req.GroupBy {
			jobAttrs[metaKey][key.Key] = stringData[key.Key]
		}
	}
...
func (d *JobsRepo) GetJobList(ctx context.Context, orgID valuer.UUID, req model.JobListRequest) (model.JobListResponse, error) {
	...
	var jobAttrs map[string]map[string]string
	var topJobGroups, allJobGroups []map[string]string
	var metaErr, topErr error

	done := make(chan struct{})
	go func() {
		jobAttrs, metaErr = d.getMetadataAttributes(ctx, req)
		done <- struct{}{}
	}()
	topJobGroups, allJobGroups, topErr = d.getTopJobGroups(ctx, orgID, req, query)
	<-done
	...
	for _, result := range formattedResponse {
		for _, row := range result.Table.Rows {
			...
			metaKey := workloadMetaKey(record.JobName, namespaceName, clusterName)
			if _, ok := jobAttrs[metaKey]; ok && record.JobName != "" {
				record.Meta = jobAttrs[metaKey]
			}
	...
	if len(topJobGroups) > 0 {
		groupByKeys := make([]string, len(req.GroupBy))
		for i, gb := range req.GroupBy {
			groupByKeys[i] = gb.Key
		}
		validGroups := make(map[string]bool, len(topJobGroups))
		for _, g := range topJobGroups {
			validGroups[buildGroupTupleKey(g, groupByKeys)] = true
		}
		filteredRecords := make([]model.JobListRecord, 0, len(topJobGroups))
