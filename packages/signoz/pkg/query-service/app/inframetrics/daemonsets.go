...
func (d *DaemonSetsRepo) getMetadataAttributes(ctx context.Context, req model.DaemonSetListRequest) (map[string]map[string]string, error) {
	...
	for _, row := range attrsListResponse {
		...
		metaKey := workloadMetaKey(daemonSetName, namespaceName, clusterName)
		if _, ok := daemonSetAttrs[metaKey]; !ok {
			daemonSetAttrs[metaKey] = map[string]string{}
		}

		for _, key := range req.GroupBy {
			daemonSetAttrs[metaKey][key.Key] = stringData[key.Key]
		}
	}
...
func (d *DaemonSetsRepo) GetDaemonSetList(ctx context.Context, orgID valuer.UUID, req model.DaemonSetListRequest) (model.DaemonSetListResponse, error) {
	...
	var daemonSetAttrs map[string]map[string]string
	var topDaemonSetGroups, allDaemonSetGroups []map[string]string
	var metaErr, topErr error

	done := make(chan struct{})
	go func() {
		daemonSetAttrs, metaErr = d.getMetadataAttributes(ctx, req)
		done <- struct{}{}
	}()
	topDaemonSetGroups, allDaemonSetGroups, topErr = d.getTopDaemonSetGroups(ctx, orgID, req, query)
	<-done
	...
	for _, result := range formattedResponse {
		for _, row := range result.Table.Rows {
			...
			metaKey := workloadMetaKey(record.DaemonSetName, namespaceName, clusterName)
			if _, ok := daemonSetAttrs[metaKey]; ok && record.DaemonSetName != "" {
				record.Meta = daemonSetAttrs[metaKey]
			}
	...
	if len(topDaemonSetGroups) > 0 {
		groupByKeys := make([]string, len(req.GroupBy))
		for i, gb := range req.GroupBy {
			groupByKeys[i] = gb.Key
		}
		validGroups := make(map[string]bool, len(topDaemonSetGroups))
		for _, g := range topDaemonSetGroups {
			validGroups[buildGroupTupleKey(g, groupByKeys)] = true
		}
		filteredRecords := make([]model.DaemonSetListRecord, 0, len(topDaemonSetGroups))
