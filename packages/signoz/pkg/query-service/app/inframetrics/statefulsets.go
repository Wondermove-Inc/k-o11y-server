...
func (d *StatefulSetsRepo) getMetadataAttributes(ctx context.Context, req model.StatefulSetListRequest) (map[string]map[string]string, error) {
	...
	for _, row := range attrsListResponse {
		...
		metaKey := workloadMetaKey(statefulSetName, namespaceName, clusterName)
		if _, ok := statefulSetAttrs[metaKey]; !ok {
			statefulSetAttrs[metaKey] = map[string]string{}
		}

		for _, key := range req.GroupBy {
			statefulSetAttrs[metaKey][key.Key] = stringData[key.Key]
		}
	}
...
func (d *StatefulSetsRepo) GetStatefulSetList(ctx context.Context, orgID valuer.UUID, req model.StatefulSetListRequest) (model.StatefulSetListResponse, error) {
	...
	var statefulSetAttrs map[string]map[string]string
	var topStatefulSetGroups, allStatefulSetGroups []map[string]string
	var metaErr, topErr error

	done := make(chan struct{})
	go func() {
		statefulSetAttrs, metaErr = d.getMetadataAttributes(ctx, req)
		done <- struct{}{}
	}()
	topStatefulSetGroups, allStatefulSetGroups, topErr = d.getTopStatefulSetGroups(ctx, orgID, req, query)
	<-done
	...
	for _, result := range formattedResponse {
		for _, row := range result.Table.Rows {
			...
			metaKey := workloadMetaKey(record.StatefulSetName, namespaceName, clusterName)
			if _, ok := statefulSetAttrs[metaKey]; ok && record.StatefulSetName != "" {
				record.Meta = statefulSetAttrs[metaKey]
			}
	...
	if len(topStatefulSetGroups) > 0 {
		groupByKeys := make([]string, len(req.GroupBy))
		for i, gb := range req.GroupBy {
			groupByKeys[i] = gb.Key
		}
		validGroups := make(map[string]bool, len(topStatefulSetGroups))
		for _, g := range topStatefulSetGroups {
			validGroups[buildGroupTupleKey(g, groupByKeys)] = true
		}
		filteredRecords := make([]model.StatefulSetListRecord, 0, len(topStatefulSetGroups))
