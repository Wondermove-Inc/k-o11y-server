...
var dotMetricMap = map[string]string{
	...
	"k8s_rollout_name":                      "k8s.rollout.name",
	"os_type":                               "os.type",
...
func getParamsForTopRollouts(req model.RolloutListRequest) (int64, string, string) {
	return getParamsForTopItems(req.Start, req.End)
}
...
// Common attribute keys used across all workload types for composite key generation
// Note: k8sNamespaceNameAttrKey is declared in namespaces.go (package-level)
var (
	k8sClusterNameAttrKey = GetDotMetrics("k8s_cluster_name")
)

// workloadMetaKey creates a composite key for workload metadata maps
// to avoid collisions when the same workload name exists in different namespaces/clusters.
func workloadMetaKey(name, namespace, cluster string) string {
	return name + "|" + namespace + "|" + cluster
}

// buildGroupTupleKey creates a composite key from a labels map using the specified ordered keys.
// Used for post-filtering to eliminate cross-product false matches from independent IN filters.
func buildGroupTupleKey(labels map[string]string, keys []string) string {
