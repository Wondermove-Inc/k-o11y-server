...
import (
	...
	"github.com/SigNoz/signoz/pkg/k8s"
	...
	ko11ycrypto "github.com/SigNoz/signoz/pkg/crypto"
...
type APIHandler struct {
	...
	rolloutsRepo     *inframetrics.RolloutsRepo
...
func NewAPIHandler(opts APIHandlerOpts) (*APIHandler, error) {
	...
	rolloutsRepo := inframetrics.NewRolloutsRepo(opts.Reader, querierv2)
...
func (aH *APIHandler) RegisterInfraMetricsRoutes(router *mux.Router, am *middleware.AuthZ) {
	...
	rolloutsSubRouter := router.PathPrefix("/api/v1/rollouts").Subrouter()
	rolloutsSubRouter.HandleFunc("/attribute_keys", am.ViewAccess(aH.getRolloutAttributeKeys)).Methods(http.MethodGet)
	rolloutsSubRouter.HandleFunc("/attribute_values", am.ViewAccess(aH.getRolloutAttributeValues)).Methods(http.MethodGet)
	rolloutsSubRouter.HandleFunc("/list", am.ViewAccess(aH.getRolloutList)).Methods(http.MethodPost)
...
func (aH *APIHandler) RegisterRoutes(router *mux.Router, am *middleware.AuthZ) {
	...
	router.HandleFunc("/api/v1/alertmanager/settings", am.AdminAccess(aH.AlertmanagerAPI.GetSettings)).Methods(http.MethodGet)
	router.HandleFunc("/api/v1/alertmanager/settings", am.AdminAccess(aH.AlertmanagerAPI.UpdateSettings)).Methods(http.MethodPut)
	...
	router.HandleFunc("/api/v1/settings/cold-storage", am.ViewAccess(aH.getColdStorageConfig)).Methods(http.MethodGet)
	router.HandleFunc("/api/v1/settings/cold-storage", am.AdminAccess(aH.updateColdStorageConfig)).Methods(http.MethodPut)
	router.HandleFunc("/api/v1/settings/lifecycle", am.ViewAccess(aH.getLifecycleConfig)).Methods(http.MethodGet)
	router.HandleFunc("/api/v1/settings/lifecycle", am.AdminAccess(aH.updateLifecycleConfig)).Methods(http.MethodPut)
	// S3 고정 경로를 {type} 패턴보다 먼저 등록 (gorilla/mux 매칭 순서)
	router.HandleFunc("/api/v1/settings/s3/activate", am.AdminAccess(aH.activateS3Storage)).Methods(http.MethodPost)
	router.HandleFunc("/api/v1/settings/s3/status", am.ViewAccess(aH.getS3Status)).Methods(http.MethodGet)
	router.HandleFunc("/api/v1/settings/s3/test", am.AdminAccess(aH.testS3Connection)).Methods(http.MethodPost)
	// Legacy: /api/v1/settings/s3 without type defaults to "warm"
	router.HandleFunc("/api/v1/settings/s3", am.ViewAccess(aH.getS3Config)).Methods(http.MethodGet)
	router.HandleFunc("/api/v1/settings/s3", am.AdminAccess(aH.updateS3Config)).Methods(http.MethodPut)
	// S3 config with type parameter
	router.HandleFunc("/api/v1/settings/s3/{type}", am.ViewAccess(aH.getS3Config)).Methods(http.MethodGet)
	router.HandleFunc("/api/v1/settings/s3/{type}", am.AdminAccess(aH.updateS3Config)).Methods(http.MethodPut)
	router.HandleFunc("/api/v1/settings/s3/{type}/test", am.AdminAccess(aH.testS3Connection)).Methods(http.MethodPost)
...
func (aH *APIHandler) getColdStorageConfig(w http.ResponseWriter, r *http.Request) {
	config, err := aH.reader.GetColdStorageConfig(r.Context())
	if err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: err}, nil)
		return
	}
	if config == nil {
		aH.WriteJSON(w, r, map[string]string{"status": "not_configured"})
		return
	}
	aH.WriteJSON(w, r, config)
}

func (aH *APIHandler) updateColdStorageConfig(w http.ResponseWriter, r *http.Request) {
	var req model.UpdateColdStorageConfigRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorBadData, Err: fmt.Errorf("invalid request body: %w", err)}, nil)
		return
	}

	if err := aH.reader.UpdateColdStorageConfig(context.Background(), &req); err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: err}, nil)
		return
	}

	aH.WriteJSON(w, r, map[string]string{"message": "cold storage config updated"})
}

func (aH *APIHandler) getLifecycleConfig(w http.ResponseWriter, r *http.Request) {
	config, err := aH.reader.GetDataLifecycleConfig(r.Context())
	if err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: err}, nil)
		return
	}
...
func (aH *APIHandler) updateLifecycleConfig(w http.ResponseWriter, r *http.Request) {
	var req struct {
	...
	ctx := r.Context()
	current, err := aH.reader.GetDataLifecycleConfig(ctx)
	if err != nil || current == nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: fmt.Errorf("lifecycle config not found")}, nil)
		return
	}
	...
	// Validation
	if hotDays < 1 {
		RespondError(w, &model.ApiError{Typ: model.ErrorBadData, Err: fmt.Errorf("hot_days must be >= 1")}, nil)
		return
	}
	if warmDays < 1 {
		RespondError(w, &model.ApiError{Typ: model.ErrorBadData, Err: fmt.Errorf("warm_days must be >= 1")}, nil)
		return
	}

	if err := aH.reader.UpdateDataLifecycleConfig(ctx, hotDays, warmDays, glacierEnabled, glacierRetentionDays); err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: err}, nil)
		return
	}

	// If glacier_enabled changed, synchronously apply TTL changes to all tiered tables
	// This runs directly (no goroutines) to avoid race conditions
	if glacierEnabled != current.GlacierEnabled {
		if err := aH.reader.ApplyGlacierTTL(ctx, glacierEnabled == 1); err != nil {
			// Non-fatal: config is saved, TTL apply partially failed
			zap.L().Warn("ApplyGlacierTTL partial failure", zap.Error(err))
		}
	}

	aH.WriteJSON(w, r, map[string]string{"message": "lifecycle config updated"})
}

func getS3TypeFromRequest(r *http.Request) string {
	vars := mux.Vars(r)
	s3Type := vars["type"]
	if s3Type == "" || s3Type == "test" {
		s3Type = "warm"
	}
	return s3Type
}

func (aH *APIHandler) getS3Config(w http.ResponseWriter, r *http.Request) {
	s3Type := getS3TypeFromRequest(r)
	config, err := aH.reader.GetS3Config(r.Context(), s3Type)
	if err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: err}, nil)
		return
	}
	...
	config.AccessKeyID = ko11ycrypto.MaskSecret(config.AccessKeyID)
	config.SecretAccessKey = ko11ycrypto.MaskSecret(config.SecretAccessKey)
	aH.WriteJSON(w, r, config)
}

func (aH *APIHandler) updateS3Config(w http.ResponseWriter, r *http.Request) {
	s3Type := getS3TypeFromRequest(r)
	var req model.UpdateS3ConfigRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorBadData, Err: fmt.Errorf("invalid request body: %w", err)}, nil)
		return
	}
	...
	ctx := r.Context()
	current, err := aH.reader.GetS3Config(ctx, s3Type)
	if err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: err}, nil)
		return
	}
	...
	// Auto-generate endpoint if empty
	if endpoint == "" && bucket != "" {
		endpoint = fmt.Sprintf("https://%s.s3.%s.amazonaws.com/data/", bucket, region)
	}

	// Encrypt credentials if provided (not masked)
	if req.AccessKeyID != nil && *req.AccessKeyID != "****" {
		encrypted, err := ko11ycrypto.Encrypt(*req.AccessKeyID)
		if err != nil {
			RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: fmt.Errorf("failed to encrypt access_key: %w", err)}, nil)
			return
		}
		accessKeyID = encrypted
	}
	if req.SecretAccessKey != nil && *req.SecretAccessKey != "****" {
		encrypted, err := ko11ycrypto.Encrypt(*req.SecretAccessKey)
		if err != nil {
			RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: fmt.Errorf("failed to encrypt secret_key: %w", err)}, nil)
			return
		}
		secretAccessKey = encrypted
	}

	if err := aH.reader.UpdateS3Config(ctx, s3Type, authMode, bucket, region, endpoint, accessKeyID, secretAccessKey, s3Enabled); err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: err}, nil)
		return
	}

	aH.WriteJSON(w, r, map[string]string{"message": "S3 config updated"})
}

func (aH *APIHandler) testS3Connection(w http.ResponseWriter, r *http.Request) {
	s3Type := getS3TypeFromRequest(r)
	var req model.TestS3ConnectionRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorBadData, Err: fmt.Errorf("invalid request body: %w", err)}, nil)
		return
	}
	...
	if req.Bucket == "" {
		RespondError(w, &model.ApiError{Typ: model.ErrorBadData, Err: fmt.Errorf("bucket is required")}, nil)
		return
	}

	ctx := r.Context()

	// Real AWS S3 HeadBucket test
	region := req.Region
	if region == "" {
		region = "ap-northeast-2"
	}
	...
	if req.AuthMode == "iam" {
		// IAM mode: use environment credentials (Instance Profile / IRSA)
		sess, sessErr = session.NewSession(&aws.Config{
			Region: aws.String(region),
		})
	} else {
		// Static mode: use provided access key + secret key
		accessKey := req.AccessKeyID
		secretKey := req.SecretAccessKey

		// If masked (****) or empty, get from DB and decrypt
		if accessKey == "" || accessKey == "****" || secretKey == "" || secretKey == "****" {
			current, err := aH.reader.GetS3Config(ctx, s3Type)
			if err == nil && current != nil {
				if accessKey == "" || accessKey == "****" {
					decrypted, decErr := ko11ycrypto.Decrypt(current.AccessKeyID)
					if decErr == nil {
						accessKey = decrypted
					} else {
						accessKey = current.AccessKeyID // fallback: might be plaintext
					}
				}
				if secretKey == "" || secretKey == "****" {
					decrypted, decErr := ko11ycrypto.Decrypt(current.SecretAccessKey)
					if decErr == nil {
						secretKey = decrypted
					} else {
						secretKey = current.SecretAccessKey
					}
				}
			}
		}
	...
	if sessErr != nil {
		_ = aH.reader.UpdateS3ConnectionTest(ctx, s3Type, false)
		aH.WriteJSON(w, r, map[string]interface{}{
			"success": false,
			"message": fmt.Sprintf("Failed to create AWS session: %v", sessErr),
		})
		return
	}
	...
	_ = aH.reader.UpdateS3ConnectionTest(ctx, s3Type, true)
	aH.WriteJSON(w, r, map[string]interface{}{
		"success": true,
		"message": fmt.Sprintf("Successfully connected to bucket '%s' in region '%s'", req.Bucket, region),
	})
}

// activateS3Storage creates a K8s Job to SSH into the CH VM and activate S3 storage.
func (aH *APIHandler) activateS3Storage(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

	// Determine mode from query param (default: activate)
	mode := r.URL.Query().Get("mode")
	if mode == "" {
		mode = "activate"
	}
	if mode != "activate" && mode != "apply" {
		RespondError(w, &model.ApiError{Typ: model.ErrorBadData, Err: fmt.Errorf("mode must be 'activate' or 'apply'")}, nil)
		return
	}

	// Verify S3 config exists in DB
	warmConfig, err := aH.reader.GetS3Config(ctx, "warm")
	if err != nil || warmConfig == nil || warmConfig.Bucket == "" {
		RespondError(w, &model.ApiError{Typ: model.ErrorBadData, Err: fmt.Errorf("S3 credentials not configured. Save S3 settings first")}, nil)
		return
	}

	// Agent mode 확인: VM 에이전트가 실행 중이면 DB INSERT만 수행 (SSH 불필요)
	agentMode, _ := aH.reader.IsAgentMode(ctx)

	if agentMode {
		// Agent mode: activate_requested=1로 INSERT → 에이전트가 폴링해서 감지 후 Activate 수행
		if err := aH.reader.UpdateS3Config(ctx, "warm",
			warmConfig.AuthMode, warmConfig.Bucket, warmConfig.Region, warmConfig.Endpoint,
			warmConfig.AccessKeyID, warmConfig.SecretAccessKey, 1, 1); err != nil {
			RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: fmt.Errorf("failed to update s3_config: %w", err)}, nil)
			return
		}
		...
		aH.WriteJSON(w, r, map[string]interface{}{
			"status":  "accepted",
			"mode":    "agent",
			"message": "S3 activation delegated to VM agent. Agent will detect the change within 30 seconds.",
		})
		return
	}

	// Legacy mode: K8s Job + SSH 방식
	k8sClient, err := k8s.NewInClusterClient()
	if err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: fmt.Errorf("K8s client initialization failed: %w", err)}, nil)
		return
	}

	// Read configuration from environment
	cfg := k8s.S3ActivateJobConfig{
		Image:            os.Getenv("S3_ACTIVATE_JOB_IMAGE"),
		CHHost:           os.Getenv("CH_HOST"),
		CHSSHUser:        os.Getenv("CH_SSH_USER"),
		CHSSHPort:        os.Getenv("CH_SSH_PORT"),
		CHPassword:       os.Getenv("CH_PASSWORD"),
		SSHKeySecretName: os.Getenv("CH_SSH_KEY_SECRET"),
		EncryptionKey:    os.Getenv("K_O11Y_ENCRYPTION_KEY"),
		Mode:             mode,
	}

	if cfg.Image == "" {
		cfg.Image = "<YOUR_REGISTRY>/tools/s3-activate:latest"
	}
	// 필수 환경변수 검증
	for _, v := range []struct{ name, value string }{
		{"CH_HOST", cfg.CHHost},
		{"CH_PASSWORD", cfg.CHPassword},
		{"K_O11Y_ENCRYPTION_KEY", cfg.EncryptionKey},
	} {
		if v.value == "" {
			RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: fmt.Errorf("%s not configured", v.name)}, nil)
			return
		}
	}
	if cfg.SSHKeySecretName == "" {
		cfg.SSHKeySecretName = "ko11y-ch-ssh-key"
	}

	jobName, err := k8sClient.CreateS3ActivateJob(ctx, cfg)
	if err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: fmt.Errorf("failed to create activation job: %w", err)}, nil)
		return
	}
	...
	aH.WriteJSON(w, r, map[string]interface{}{
		...
		"mode":    "legacy",
		"job_id":  jobName,
		"message": "S3 activation started via K8s Job. Poll /api/v1/settings/s3/status for progress.",
	})
}

// getS3Status returns the current S3 activation status.
func (aH *APIHandler) getS3Status(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

	status, err := aH.reader.GetS3Status(ctx)
	if err != nil {
		RespondError(w, &model.ApiError{Typ: model.ErrorInternal, Err: err}, nil)
		return
	}
	...
	// Agent mode 상태 추가
	agentMode, _ := aH.reader.IsAgentMode(ctx)
	status["agent_mode"] = agentMode

	if agentMode {
		// Agent mode: activate_requested + s3_active로 정확한 상태 판단
		s3Active, _ := status["s3_active"].(bool)
		if s3Active {
			status["activation_status"] = "success"
		} else {
			// activate_requested를 DB에서 확인
			warmConfig, _ := aH.reader.GetS3Config(ctx, "warm")
			if warmConfig != nil && warmConfig.ActivateRequested == 1 {
				status["activation_status"] = "running"
			}
			// activate_requested=0이면 "idle" 유지 (Save만 한 상태)
		}
	} else {
		// Legacy mode: K8s Job status로 판단
		k8sClient, k8sErr := k8s.NewInClusterClient()
		if k8sErr == nil {
			jobStatus, jobErr := k8sClient.GetS3JobStatus(ctx, "")
			if jobErr == nil && jobStatus != nil {
				if jobStatus.Active {
					status["activation_status"] = "running"
				...
				status["job_name"] = jobStatus.Name
				status["job_message"] = jobStatus.Message
			}
		}
	}

	aH.WriteJSON(w, r, status)
}
