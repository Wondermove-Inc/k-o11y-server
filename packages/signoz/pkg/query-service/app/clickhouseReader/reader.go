...
func (r *ClickHouseReader) setTTLLogs(ctx context.Context, orgID string, params *model.TTLParams) (*model.SetTTLResponseItem, *model.ApiError) {
	...
	glacierOn := r.isGlacierEnabled(ctx)

	// TTL query for logs_v2 table
	var ttlLogsV2 string
	if glacierOn && len(params.ColdStorageVolume) > 0 {
		// Glacier ON: no DELETE
		ttlLogsV2 = fmt.Sprintf(
			"ALTER TABLE %v ON CLUSTER %s MODIFY TTL toDateTime(timestamp / 1000000000)"+
				" + INTERVAL %v SECOND TO VOLUME '%s'",
			tableNameArray[0], r.cluster, params.ToColdStorageDuration, params.ColdStorageVolume)
		ttlLogsV2 += fmt.Sprintf(", toDateTime(timestamp / 1000000000)"+
			" + INTERVAL %v SECOND RECOMPRESS CODEC(ZSTD(3))",
			params.ToColdStorageDuration)
	} else {
		ttlLogsV2 = fmt.Sprintf(
			"ALTER TABLE %v ON CLUSTER %s MODIFY TTL toDateTime(timestamp / 1000000000) + "+
	...
	if glacierOn && len(params.ColdStorageVolume) > 0 {
		ttlLogsV2Resource = fmt.Sprintf(
			"ALTER TABLE %v ON CLUSTER %s MODIFY TTL toDateTime(seen_at_ts_bucket_start) + toIntervalSecond(1800) + "+
				"INTERVAL %v SECOND TO VOLUME '%s'",
			tableNameArray[1], r.cluster, params.ToColdStorageDuration, params.ColdStorageVolume)
	...
	// AttributeKeys/ResourceKeys: always DELETE (metadata, no Glacier archival needed)
...
func (r *ClickHouseReader) setTTLTraces(ctx context.Context, orgID string, params *model.TTLParams) (*model.SetTTLResponseItem, *model.ApiError) {
	...
	// TTL query templates
	var ttlV2, ttlV2Resource string
	if glacierOn {
		// Glacier ON: no DELETE — cron will DROP PARTITION after backup
		ttlV2 = "ALTER TABLE %s ON CLUSTER %s MODIFY TTL "
		ttlV2Resource = "ALTER TABLE %s ON CLUSTER %s MODIFY TTL "
	} else {
	...
	for _, distributedTableName := range tableNames {
		go func(distributedTableName string) {
			...
			var req string
			if glacierOn && len(params.ColdStorageVolume) > 0 && !strings.HasSuffix(distributedTableName, r.spanAttributesKeysTable) {
				// Glacier ON: no DELETE, only TO VOLUME + RECOMPRESS
				if strings.HasSuffix(distributedTableName, r.traceResourceTableV3) {
					req = fmt.Sprintf("ALTER TABLE %s ON CLUSTER %s MODIFY TTL toDateTime(seen_at_ts_bucket_start) + toIntervalSecond(1800)"+
						" + INTERVAL %v SECOND TO VOLUME '%s'",
						tableName, r.cluster, params.ToColdStorageDuration, params.ColdStorageVolume)
					req += fmt.Sprintf(", toDateTime(seen_at_ts_bucket_start) + toIntervalSecond(1800)"+
						" + INTERVAL %v SECOND RECOMPRESS CODEC(ZSTD(3))",
						params.ToColdStorageDuration)
				} else {
					req = fmt.Sprintf("ALTER TABLE %s ON CLUSTER %s MODIFY TTL toDateTime(%s)"+
						" + INTERVAL %v SECOND TO VOLUME '%s'",
						tableName, r.cluster, timestamp, params.ToColdStorageDuration, params.ColdStorageVolume)
					req += fmt.Sprintf(", toDateTime(%s) + INTERVAL %v SECOND RECOMPRESS CODEC(ZSTD(3))",
						timestamp, params.ToColdStorageDuration)
				}
			} else {
				// Glacier OFF: include DELETE
...
func (r *ClickHouseReader) SetTTLV2(ctx context.Context, orgID string, params *model.CustomRetentionTTLParams) (*model.CustomRetentionTTLResponse, error) {
	...
	if len(params.ColdStorageVolume) > 0 && coldStorageDuration > 0 {
		...
		queries = append(queries, fmt.Sprintf(`ALTER TABLE %s ON CLUSTER %s MODIFY TTL toDateTime(timestamp / 1000000000) + toIntervalDay(_retention_days) DELETE, toDateTime(timestamp / 1000000000) + toIntervalDay(_retention_days_cold) TO VOLUME '%s', toDateTime(timestamp / 1000000000) + toIntervalDay(_retention_days_cold) RECOMPRESS CODEC(ZSTD(3)) SETTINGS materialize_ttl_after_modify=0`,
	...
	if len(params.ColdStorageVolume) > 0 && coldStorageDuration > 0 {
		...
		resourceQueries = append(resourceQueries, fmt.Sprintf(`ALTER TABLE %s ON CLUSTER %s MODIFY TTL toDateTime(seen_at_ts_bucket_start) + toIntervalSecond(1800) + toIntervalDay(_retention_days) DELETE, toDateTime(seen_at_ts_bucket_start) + toIntervalSecond(1800) + toIntervalDay(_retention_days_cold) TO VOLUME '%s', toDateTime(seen_at_ts_bucket_start) + toIntervalSecond(1800) + toIntervalDay(_retention_days_cold) RECOMPRESS CODEC(ZSTD(3)) SETTINGS materialize_ttl_after_modify=0`,
...
func (r *ClickHouseReader) SetTTL(ctx context.Context, orgID string, params *model.TTLParams) (*model.SetTTLResponseItem, *model.ApiError) {
	// Validate DELETE TTL against cold storage min_delete_retention_days
	if err := r.validateRetentionAgainstColdStorage(ctx, params.DelDuration); err != nil {
		...
		if glacierOn && len(params.ColdStorageVolume) > 0 {
			...
			req = fmt.Sprintf(
				"ALTER TABLE %v ON CLUSTER %s MODIFY TTL toDateTime(toUInt32(%s / 1000), 'UTC')"+
					" + INTERVAL %v SECOND TO VOLUME '%s'",
				tableName, r.cluster, timeColumn, params.ToColdStorageDuration, params.ColdStorageVolume)
			req += fmt.Sprintf(", toDateTime(toUInt32(%s / 1000), 'UTC')"+
				" + INTERVAL %v SECOND RECOMPRESS CODEC(ZSTD(3))",
				timeColumn, params.ToColdStorageDuration)
		} else {
			// Glacier OFF or no S3: include DELETE
			req = fmt.Sprintf(
				"ALTER TABLE %v ON CLUSTER %s MODIFY TTL toDateTime(toUInt32(%s / 1000), 'UTC') + "+
...
func (r *ClickHouseReader) setColdStorage(ctx context.Context, tableName string, coldStorageVolume string) *model.ApiError {
	...
	if len(coldStorageVolume) > 0 {
		...
		// Set merge_with_ttl_timeout to 24h (86400s) to prevent excessive TTL merge IO on S3.
		// Default value (0) triggers TTL check on every merge, causing S3 IO spikes.
		ttlTimeoutReq := fmt.Sprintf("ALTER TABLE %s ON CLUSTER %s MODIFY SETTING merge_with_ttl_timeout = 86400", tableName, r.cluster)
		zap.L().Info("Setting merge_with_ttl_timeout", zap.String("request", ttlTimeoutReq))
		if err := r.db.Exec(ctx, ttlTimeoutReq); err != nil {
			zap.L().Warn("failed to set merge_with_ttl_timeout (non-fatal)", zap.String("table", tableName), zap.Error(err))
		}
	}
	return nil
}
...
// isGlacierEnabled checks if Glacier archival is active from data_lifecycle_config.
// Returns false if config is not available (safe default: use TTL DELETE).
func (r *ClickHouseReader) isGlacierEnabled(ctx context.Context) bool {
	config, err := r.GetDataLifecycleConfig(ctx)
	if err != nil || config == nil {
		return false
	}
	return config.GlacierEnabled == 1
}

// ApplyGlacierTTL directly modifies TTL for all 17 tiered tables based on glacier status.
// When glacierEnabled=true: removes DELETE from TTL (keeps MOVE + RECOMPRESS)
// When glacierEnabled=false: adds DELETE back to TTL
// This runs synchronously (no goroutines) to avoid race conditions.
// ALTER TABLE MODIFY TTL with materialize_ttl_after_modify=0 is nearly instant (metadata only).
func (r *ClickHouseReader) ApplyGlacierTTL(ctx context.Context, glacierEnabled bool) error {
	config, err := r.GetDataLifecycleConfig(ctx)
	if err != nil || config == nil {
		return fmt.Errorf("cannot read lifecycle config")
	}

	hotSeconds := int64(config.HotDays) * 86400
	totalSeconds := int64(config.HotDays+config.WarmDays) * 86400

	// 17 tiered tables with their timestamp expressions
	type tableInfo struct {
		name   string
		tsExpr string // timestamp expression for TTL
	}

	tables := []tableInfo{
		// Logs
		{name: "signoz_logs.logs_v2", tsExpr: "toDateTime(timestamp / 1000000000)"},
		{name: "signoz_logs.logs_v2_resource", tsExpr: "toDateTime(seen_at_ts_bucket_start) + toIntervalSecond(1800)"},
		// Metrics
		{name: "signoz_metrics.samples_v4", tsExpr: "toDateTime(toUInt32(unix_milli / 1000), 'UTC')"},
		{name: "signoz_metrics.samples_v4_agg_5m", tsExpr: "toDateTime(toUInt32(unix_milli / 1000), 'UTC')"},
		{name: "signoz_metrics.samples_v4_agg_30m", tsExpr: "toDateTime(toUInt32(unix_milli / 1000), 'UTC')"},
		{name: "signoz_metrics.exp_hist", tsExpr: "toDateTime(toUInt32(unix_milli / 1000), 'UTC')"},
		{name: "signoz_metrics.time_series_v4", tsExpr: "toDateTime(toUInt32(unix_milli / 1000), 'UTC')"},
		{name: "signoz_metrics.time_series_v4_6hrs", tsExpr: "toDateTime(toUInt32(unix_milli / 1000), 'UTC')"},
		{name: "signoz_metrics.time_series_v4_1day", tsExpr: "toDateTime(toUInt32(unix_milli / 1000), 'UTC')"},
		{name: "signoz_metrics.time_series_v4_1week", tsExpr: "toDateTime(toUInt32(unix_milli / 1000), 'UTC')"},
		// Traces
		{name: "signoz_traces.signoz_index_v3", tsExpr: "toDateTime(timestamp)"},
		{name: "signoz_traces.traces_v3_resource", tsExpr: "toDateTime(seen_at_ts_bucket_start) + toIntervalSecond(1800)"},
		{name: "signoz_traces.signoz_error_index_v2", tsExpr: "toDateTime(timestamp)"},
		{name: "signoz_traces.usage_explorer", tsExpr: "toDateTime(timestamp)"},
		{name: "signoz_traces.dependency_graph_minutes_v2", tsExpr: "toDateTime(timestamp)"},
		{name: "signoz_traces.trace_summary", tsExpr: "toDateTime(end)"},
		// span_attributes_keys has no TTL timestamp — skip
	}
	...
	for _, tbl := range tables {
		var req string
		if glacierEnabled {
			// Glacier ON: MOVE + RECOMPRESS only (no DELETE)
			req = fmt.Sprintf(
				"ALTER TABLE %s ON CLUSTER %s MODIFY TTL %s + INTERVAL %d SECOND TO VOLUME 's3', "+
					"%s + INTERVAL %d SECOND RECOMPRESS CODEC(ZSTD(3)) "+
					"SETTINGS materialize_ttl_after_modify = 0",
				tbl.name, r.cluster, tbl.tsExpr, hotSeconds, tbl.tsExpr, hotSeconds)
		} else {
			// Glacier OFF: DELETE + MOVE + RECOMPRESS
			req = fmt.Sprintf(
				"ALTER TABLE %s ON CLUSTER %s MODIFY TTL %s + INTERVAL %d SECOND DELETE, "+
					"%s + INTERVAL %d SECOND TO VOLUME 's3', "+
					"%s + INTERVAL %d SECOND RECOMPRESS CODEC(ZSTD(3)) "+
					"SETTINGS materialize_ttl_after_modify = 0",
				tbl.name, r.cluster, tbl.tsExpr, totalSeconds, tbl.tsExpr, hotSeconds, tbl.tsExpr, hotSeconds)
		}

		zap.L().Info("ApplyGlacierTTL", zap.String("table", tbl.name), zap.Bool("glacier", glacierEnabled), zap.String("query", req))

		if err := r.db.Exec(ctx, req); err != nil {
			zap.L().Error("ApplyGlacierTTL failed", zap.String("table", tbl.name), zap.Error(err))
			failCount++
	...
	zap.L().Info("ApplyGlacierTTL completed", zap.Int("success", successCount), zap.Int("failed", failCount))

	if failCount > 0 {
		return fmt.Errorf("ApplyGlacierTTL: %d/%d tables failed", failCount, successCount+failCount)
	}
	return nil
}

// GetDataLifecycleConfig reads the unified lifecycle config from ko11y.data_lifecycle_config.
// Returns nil (not error) if the table or database doesn't exist.
func (r *ClickHouseReader) GetDataLifecycleConfig(ctx context.Context) (*model.DataLifecycleConfig, error) {
	config := model.DataLifecycleConfig{}
	...
	if err := r.db.QueryRow(ctx, query).ScanStruct(&config); err != nil {
		if strings.Contains(err.Error(), "UNKNOWN_DATABASE") || strings.Contains(err.Error(), "UNKNOWN_TABLE") {
			return nil, nil // Not configured
		}
		return nil, fmt.Errorf("failed to read data lifecycle config: %w", err)
	}
	return &config, nil
}

// UpdateDataLifecycleConfig updates the data lifecycle config with partial merge.
func (r *ClickHouseReader) UpdateDataLifecycleConfig(ctx context.Context, hotDays, warmDays int32, glacierEnabled uint8, glacierRetentionDays int32) error {
	current, err := r.GetDataLifecycleConfig(ctx)
	if err != nil || current == nil {
		return fmt.Errorf("lifecycle config not found")
	}
	...
	query := fmt.Sprintf(
		...
		VALUES ('global', %d, %d, %d, %d, %d, 'signoz-ui', %d)`,
		hotDays, warmDays, glacierEnabled, glacierRetentionDays,
		current.BackupFrequencyHours, time.Now().Unix(),
	)
...
// GetSSOAllowedTenants reads all allowed tenant IDs from ko11y.sso_config.
// Returns empty slice if table doesn't exist or has no rows.
func (r *ClickHouseReader) GetSSOAllowedTenants(ctx context.Context) ([]model.SSOAllowedTenant, error) {
	query := `SELECT tenant_id, toString(locked_at) as locked_at, locked_by, version
		FROM ko11y.sso_config FINAL
		ORDER BY locked_at ASC`

	var tenants []model.SSOAllowedTenant
	if err := r.db.Select(ctx, &tenants, query); err != nil {
		errStr := err.Error()
		if strings.Contains(errStr, "UNKNOWN_DATABASE") || strings.Contains(errStr, "UNKNOWN_TABLE") {
			return nil, nil
		}
		return nil, fmt.Errorf("failed to read SSO config: %w", err)
	}
	return tenants, nil
}

// InsertSSOAllowedTenant adds a tenant_id to ko11y.sso_config (auto-lock on first login).
func (r *ClickHouseReader) InsertSSOAllowedTenant(ctx context.Context, tenantID, lockedBy string) error {
	query := fmt.Sprintf(
		`INSERT INTO ko11y.sso_config (tenant_id, locked_by, version)
		VALUES ('%s', '%s', %d)`,
		tenantID, lockedBy, time.Now().Unix(),
	)
	if err := r.db.Exec(ctx, query); err != nil {
		return fmt.Errorf("failed to insert SSO allowed tenant: %w", err)
	}
	return nil
}

// GetS3Config reads the S3 storage configuration from ko11y.s3_config.
// configId: "warm", "cold", or "global" (legacy fallback)
func (r *ClickHouseReader) GetS3Config(ctx context.Context, configId ...string) (*model.S3Config, error) {
	id := "warm" // default
	...
	query := fmt.Sprintf(`SELECT config_id, auth_mode, bucket, region, endpoint,
		access_key_id, secret_access_key, s3_enabled, activate_requested,
		connection_tested, toString(connection_tested_at) as connection_tested_at,
		updated_by, toString(updated_at) as updated_at, version
		FROM ko11y.s3_config FINAL
		WHERE config_id = '%s' LIMIT 1`, id)

	if err := r.db.QueryRow(ctx, query).ScanStruct(&config); err != nil {
		errStr := err.Error()
		if strings.Contains(errStr, "UNKNOWN_DATABASE") || strings.Contains(errStr, "UNKNOWN_TABLE") || strings.Contains(errStr, "no rows") {
			// Fallback to 'global' if warm/cold not found
			if id != "global" {
				return r.GetS3Config(ctx, "global")
			}
			return nil, nil
		}
...
// UpdateS3Config updates the S3 storage configuration.
// Uses parameterized query to prevent credentials from appearing in logs.
func (r *ClickHouseReader) UpdateS3Config(ctx context.Context, configId, authMode, bucket, region, endpoint, accessKeyID, secretAccessKey string, s3Enabled uint8, activateRequested ...uint8) error {
	if configId == "" {
		configId = "warm"
	}
	var actReq uint8 = 0
	if len(activateRequested) > 0 {
		actReq = activateRequested[0]
	}
	query := `INSERT INTO ko11y.s3_config
		(config_id, auth_mode, bucket, region, endpoint, access_key_id, secret_access_key,
		 s3_enabled, activate_requested, updated_by, version)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`

	if err := r.db.Exec(ctx, query,
		configId, authMode, bucket, region, endpoint, accessKeyID, secretAccessKey,
		s3Enabled, actReq, "signoz-ui", uint64(time.Now().Unix()),
	); err != nil {
		return fmt.Errorf("failed to update S3 config: %w", err)
	}
	return nil
}

// UpdateS3ConnectionTest updates the connection test result.
func (r *ClickHouseReader) UpdateS3ConnectionTest(ctx context.Context, configId string, success bool) error {
	if configId == "" {
		configId = "warm"
	}
	var tested uint8
	...
	current, err := r.GetS3Config(ctx, configId)
	if err != nil || current == nil {
		return fmt.Errorf("S3 config not found for %s", configId)
	}

	query := `INSERT INTO ko11y.s3_config
		(config_id, auth_mode, bucket, region, endpoint, access_key_id, secret_access_key,
		 s3_enabled, connection_tested, connection_tested_at, updated_by, version)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, now(), ?, ?)`

	if err := r.db.Exec(ctx, query,
		configId, current.AuthMode, current.Bucket, current.Region, current.Endpoint,
		current.AccessKeyID, current.SecretAccessKey, current.S3Enabled,
		tested, "signoz-ui", uint64(time.Now().Unix()),
	); err != nil {
		return fmt.Errorf("failed to update connection test: %w", err)
	}
	return nil
}

// IsAgentMode checks if a CH VM agent is running by querying ko11y.agent_status.
// Returns true if an agent has sent a heartbeat within the last 90 seconds.
func (r *ClickHouseReader) IsAgentMode(ctx context.Context) (bool, error) {
	query := `SELECT count() FROM ko11y.agent_status FINAL
	          WHERE last_heartbeat > now64(3) - toIntervalSecond(90)`

	var count uint64
	if err := r.db.QueryRow(ctx, query).Scan(&count); err != nil {
		// 테이블이 없으면 agent_mode=false (backward compat)
		return false, nil
	}
	return count > 0, nil
}

// GetS3Status returns the current S3 activation status.
// s3_configured: s3_config에 warm 행이 있고 s3_enabled=1
// s3_active: CH system.disks에 S3 타입 디스크 존재
// pending_restart: s3_configured && !s3_active (동적 판단)
func (r *ClickHouseReader) GetS3Status(ctx context.Context) (map[string]interface{}, error) {
	// Check s3_configured
	s3Configured := false
	warmConfig, err := r.GetS3Config(ctx, "warm")
	if err == nil && warmConfig != nil && warmConfig.S3Enabled == 1 && warmConfig.Bucket != "" {
		s3Configured = true
	}

	// Check s3_active (S3 디스크가 CH에 등록되어 있는지)
	s3Active := false
	var diskCount uint64
	diskQuery := "SELECT count() FROM system.disks WHERE type = 's3'"
	if err := r.db.QueryRow(ctx, diskQuery).Scan(&diskCount); err == nil && diskCount > 0 {
		s3Active = true
	}
	...
	return map[string]interface{}{
		...
		"s3_active":         s3Active,
		"pending_restart":   s3Configured && !s3Active,
		"activation_status": "idle",
	}, nil
}

// GetColdStorageConfig reads the global cold storage config from ko11y.cold_storage_config.
// Returns nil config (not error) if the table or database doesn't exist.
func (r *ClickHouseReader) GetColdStorageConfig(ctx context.Context) (*model.ColdStorageConfig, error) {
	config := model.ColdStorageConfig{}
	query := `SELECT signal_type, glacier_enabled, glacier_retention_days,
	...
	if err := r.db.QueryRow(ctx, query).ScanStruct(&config); err != nil {
		if strings.Contains(err.Error(), "UNKNOWN_DATABASE") || strings.Contains(err.Error(), "UNKNOWN_TABLE") {
			return nil, nil // Glacier not configured
		}
		return nil, fmt.Errorf("failed to read cold storage config: %w", err)
	}
	return &config, nil
}

// UpdateColdStorageConfig updates the global cold storage config in ko11y.cold_storage_config.
func (r *ClickHouseReader) UpdateColdStorageConfig(ctx context.Context, req *model.UpdateColdStorageConfigRequest) error {
	// Read current config
	current, err := r.GetColdStorageConfig(ctx)
	if err != nil {
		return err
	}
	if current == nil {
		return fmt.Errorf("cold storage config not found (ko11y.cold_storage_config table missing)")
	}

	// Merge: apply only non-nil fields from request
	...
	if glacierRetentionDays < 0 {
		return fmt.Errorf("glacier_retention_days must be >= 0 (0 = unlimited)")
	}
	if minDeleteRetentionDays < 1 {
		return fmt.Errorf("min_delete_retention_days must be >= 1")
	}

	query := fmt.Sprintf(`INSERT INTO ko11y.cold_storage_config
		(signal_type, glacier_enabled, glacier_retention_days, backup_frequency_hours,
		 min_delete_retention_days, updated_by, version)
		VALUES ('global', %d, %d, %d, %d, 'signoz-ui', toUnixTimestamp(now()))`,
		glacierEnabled, glacierRetentionDays, backupFrequencyHours, minDeleteRetentionDays)

	if err := r.db.Exec(ctx, query); err != nil {
		return fmt.Errorf("failed to update cold storage config: %w", err)
	}
	return nil
}

// validateRetentionAgainstColdStorage checks that DELETE TTL is not shorter than
// the minimum required by cold storage backup schedule.
func (r *ClickHouseReader) validateRetentionAgainstColdStorage(ctx context.Context, delDurationSeconds int64) error {
	config, err := r.GetColdStorageConfig(ctx)
	if err != nil || config == nil {
		return nil // Glacier not configured, skip validation
	}
	if config.GlacierEnabled == 0 {
		return nil
	}

	deleteRetentionDays := delDurationSeconds / 86400
	if deleteRetentionDays < int64(config.MinDeleteRetentionDays) {
		return fmt.Errorf("DELETE retention (%d days) must be >= %d days (glacier backup safety margin)",
			deleteRetentionDays, config.MinDeleteRetentionDays)
	}
	return nil
}
...
// parseTTLFromEngineFullStr parses DELETE and MOVE TTL values (in hours) from
// ClickHouse engine_full string. Supports both toIntervalSecond(N) and
// toIntervalDay(N) formats.
func parseTTLFromEngineFullStr(queryResp string) (int, int) {
	zap.L().Info("Parsing TTL from: ", zap.String("queryResp", queryResp))

	deleteSecExp := regexp.MustCompile(`toIntervalSecond\(([0-9]*)\)`)
	deleteDayExp := regexp.MustCompile(`toIntervalDay\(([0-9]*)\)`)
	moveSecExp := regexp.MustCompile(`toIntervalSecond\(([0-9]*)\) TO VOLUME`)
	moveDayExp := regexp.MustCompile(`toIntervalDay\(([0-9]*)\) TO VOLUME`)

	var delTTL, moveTTL int = -1, -1

	if m := deleteSecExp.FindStringSubmatch(queryResp); len(m) > 1 {
		seconds_int, err := strconv.Atoi(m[1])
		if err != nil {
			return -1, -1
		}
		delTTL = seconds_int / 3600
	} else if m := deleteDayExp.FindStringSubmatch(queryResp); len(m) > 1 {
		days_int, err := strconv.Atoi(m[1])
		if err != nil {
			return -1, -1
		}
		delTTL = days_int * 24
	...
	if m := moveSecExp.FindStringSubmatch(queryResp); len(m) > 1 {
		seconds_int, err := strconv.Atoi(m[1])
		if err != nil {
			return -1, -1
		}
		moveTTL = seconds_int / 3600
	} else if m := moveDayExp.FindStringSubmatch(queryResp); len(m) > 1 {
		days_int, err := strconv.Atoi(m[1])
		if err != nil {
			return -1, -1
		}
...
func (r *ClickHouseReader) GetTTL(ctx context.Context, orgID string, ttlParams *model.GetTTLParams) (*model.GetTTLResponseItem, *model.ApiError) {
	...
	parseTTL := parseTTLFromEngineFullStr
