...
type Reader interface {
	...
	// S3 Config (ko11y.s3_config)
	GetS3Config(ctx context.Context, configId ...string) (*model.S3Config, error)
	UpdateS3Config(ctx context.Context, configId, authMode, bucket, region, endpoint, accessKeyID, secretAccessKey string, s3Enabled uint8, activateRequested ...uint8) error
	UpdateS3ConnectionTest(ctx context.Context, configId string, success bool) error
	GetS3Status(ctx context.Context) (map[string]interface{}, error)

	// Agent Mode (ko11y.agent_status)
	IsAgentMode(ctx context.Context) (bool, error)

	// Cold Storage Config (ko11y.cold_storage_config) — legacy
	GetColdStorageConfig(ctx context.Context) (*model.ColdStorageConfig, error)
	UpdateColdStorageConfig(ctx context.Context, req *model.UpdateColdStorageConfigRequest) error

	// SSO Config (ko11y.sso_config) — Tenant Auto-Lock
	GetSSOAllowedTenants(ctx context.Context) ([]model.SSOAllowedTenant, error)
	InsertSSOAllowedTenant(ctx context.Context, tenantID, lockedBy string) error

	// Data Lifecycle Config (ko11y.data_lifecycle_config) — Single Source of Truth
	GetDataLifecycleConfig(ctx context.Context) (*model.DataLifecycleConfig, error)
	UpdateDataLifecycleConfig(ctx context.Context, hotDays, warmDays int32, glacierEnabled uint8, glacierRetentionDays int32) error
	ApplyGlacierTTL(ctx context.Context, glacierEnabled bool) error
