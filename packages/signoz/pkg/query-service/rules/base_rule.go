...
func (r *BaseRule) GeneratorURLForAlert(ctx context.Context) string {
	return ruletypes.PrepareRuleGeneratorURL(r.ID(), r.alertBaseURL(ctx))
}

func (r *BaseRule) alertBaseURL(ctx context.Context) string {
	if ctx == nil || r.sqlstore == nil {
		return r.fallbackAlertBaseURL()
	}

	preference := new(preferencetypes.StorableOrgPreference)
	err := r.sqlstore.
		BunDBCtx(ctx).
		NewSelect().
		Model(preference).
		Where("org_id = ?", r.orgID).
		Where("preference_id = ?", preferencetypes.NameAlertBaseURL).
		Limit(1).
		Scan(ctx)
	if err == nil {
		value, valueErr := preferencetypes.NewValueFromString(preference.Value, preferencetypes.ValueTypeString)
		if valueErr == nil {
			stringValue, stringErr := value.StringValue()
			if stringErr == nil {
				normalized, normalizeErr := preferencetypes.NormalizeAlertBaseURL(stringValue)
				if normalizeErr == nil {
					return normalized
				}
			}
...
func (r *BaseRule) fallbackAlertBaseURL() string {
	if r.source != "" {
		parsed, err := url.Parse(r.source)
		if err == nil && parsed.Scheme != "" && parsed.Hostname() != "" {
			if parsed.Port() != "" {
				return fmt.Sprintf("%s://%s:%s", parsed.Scheme, parsed.Hostname(), parsed.Port())
			}
			return fmt.Sprintf("%s://%s", parsed.Scheme, parsed.Hostname())
		}
	}

	return preferencetypes.DefaultAlertBaseURL
}
