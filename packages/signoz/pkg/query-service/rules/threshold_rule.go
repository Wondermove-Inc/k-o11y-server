...
func (r *ThresholdRule) Eval(ctx context.Context, ts time.Time) (interface{}, error) {
	...
	for _, smpl := range res {
		...
		lb.Set(labels.RuleSourceLabel, r.GeneratorURLForAlert(ctx))
		...
		case ruletypes.AlertTypeTraces:
			...
			baseURL := r.alertBaseURL(ctx)
			if link != "" && baseURL != "" {
				r.logger.InfoContext(ctx, "adding traces link to annotations", "link", fmt.Sprintf("%s/traces-explorer?%s", baseURL, link))
				annotations = append(annotations, labels.Label{Name: "related_traces", Value: fmt.Sprintf("%s/traces-explorer?%s", baseURL, link)})
			}
		case ruletypes.AlertTypeLogs:
			link := r.prepareLinksToLogs(ctx, ts, smpl.Metric)
			baseURL := r.alertBaseURL(ctx)
			if link != "" && baseURL != "" {
				r.logger.InfoContext(ctx, "adding logs link to annotations", "link", fmt.Sprintf("%s/logs/logs-explorer?%s", baseURL, link))
				annotations = append(annotations, labels.Label{Name: "related_logs", Value: fmt.Sprintf("%s/logs/logs-explorer?%s", baseURL, link)})
			}
		}
		...
		alerts[h] = &ruletypes.Alert{
			...
			Value:             smpl.V,
			GeneratorURL:      r.GeneratorURLForAlert(ctx),
