...
// KO11ySSOConfig holds configuration for K-O11y JWT SSO integration.
// When Enabled is true, the AuthN middleware will accept K-O11y JWTs (RS256) as a fallback.
type KO11ySSOConfig struct {
	Enabled        bool     `mapstructure:"enabled"`
	JWTPublicKey   string   `mapstructure:"jwt_public_key"`
	JWTIssuer      string   `mapstructure:"jwt_issuer"`
	DefaultRole    string   `mapstructure:"default_role"`
	AllowedTenants []string `mapstructure:"-"` // tenant_id ACL (empty = allow all)
}

// Validate implements factory.Config.
func (c KO11ySSOConfig) Validate() error {
	if c.Enabled && c.JWTPublicKey == "" {
		return errors.New(errors.TypeInvalidInput, errors.CodeInvalidInput, "K_O11Y_JWT_PUBLIC_KEY is required when K-O11y SSO is enabled")
	}
	return nil
}
...
type Config struct {
	...
	// K-O11y SSO config (not managed by factory, loaded from env vars)
	KO11ySSO KO11ySSOConfig `mapstructure:"-"`
}
...
func (df *DeprecatedFlags) RegisterFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(&df.Cluster, "cluster", "", "(cluster name - defaults to 'cluster')")
...
func mergeAndEnsureBackwardCompatibility(ctx context.Context, logger *slog.Logger, config *Config, deprecatedFlags DeprecatedFlags) {
	...
	// K-O11y SSO configuration (loaded from environment variables)
	if os.Getenv("K_O11Y_SSO_ENABLED") == "true" {
		config.KO11ySSO.Enabled = true
		config.KO11ySSO.JWTPublicKey = os.Getenv("K_O11Y_JWT_PUBLIC_KEY")
		config.KO11ySSO.JWTIssuer = os.Getenv("K_O11Y_JWT_ISSUER")
		config.KO11ySSO.DefaultRole = os.Getenv("K_O11Y_DEFAULT_ROLE")
		if config.KO11ySSO.DefaultRole == "" {
			config.KO11ySSO.DefaultRole = "EDITOR"
		}
		if config.KO11ySSO.JWTPublicKey == "" {
			logger.ErrorContext(ctx, "K_O11Y_SSO_ENABLED is true but K_O11Y_JWT_PUBLIC_KEY is not set")
			config.KO11ySSO.Enabled = false
		} else {
			logger.InfoContext(ctx, "K-O11y SSO enabled (RS256)")
		}
		// Tenant-based access control:
		//   (empty) = auto-lock on first login (default, most secure)
		//   "*"     = allow all tenants (internal environments)
		//   "t1,t2" = explicit list
		if tenants := os.Getenv("K_O11Y_ALLOWED_TENANTS"); tenants != "" {
			for _, t := range strings.Split(tenants, ",") {
				if trimmed := strings.TrimSpace(t); trimmed != "" {
					config.KO11ySSO.AllowedTenants = append(config.KO11ySSO.AllowedTenants, trimmed)
				}
			}
			logger.InfoContext(ctx, "K-O11y SSO tenant ACL configured", "allowed_tenants", config.KO11ySSO.AllowedTenants)
		}
	}
}
