...
func (value Value) StringValue() (string, error) {
	if value.valueType != ValueTypeString {
		return "", errors.Newf(errors.TypeInvalidInput, errors.CodeInvalidInput, "value type is not string: %s", value.valueType)
	}

	stringValue, ok := value.goValue.(string)
	if !ok {
