...
func TestGetDefaultNotificationConfigUsesRouteRepeatInterval(t *testing.T) {
	config := GetDefaultNotificationConfig()

	assert.Equal(t, time.Duration(0), config.Renotify.RenotifyInterval)
	assert.Equal(t, time.Duration(0), config.Renotify.NoDataInterval)
	assert.Contains(t, config.NotificationGroup, model.LabelName(DefaultGroupBy))
}
