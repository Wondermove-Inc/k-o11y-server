...
func applyGlobalSMTPDefaults(receiver *config.Receiver, globalConfig *config.GlobalConfig) {
	if receiver == nil || globalConfig == nil {
		return
	}

	defaultGlobal := config.DefaultGlobalConfig()
	for i := range receiver.EmailConfigs {
		emailConfig := receiver.EmailConfigs[i]
		if emailConfig == nil {
			continue
		}
		if globalConfig.SMTPFrom != "" && emailConfig.From != globalConfig.SMTPFrom {
			emailConfig.From = globalConfig.SMTPFrom
		}
		if emailConfig.Smarthost == (config.HostPort{}) ||
			emailConfig.Smarthost == defaultGlobal.SMTPSmarthost {
			emailConfig.Smarthost = globalConfig.SMTPSmarthost
		}
	}
}
...
func (c *Config) CreateReceiver(receiver config.Receiver) error {
	...
	applyGlobalSMTPDefaults(&receiver, c.alertmanagerConfig.Global)
