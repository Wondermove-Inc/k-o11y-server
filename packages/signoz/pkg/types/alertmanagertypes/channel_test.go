...
func TestNewConfigFromChannels(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "EmailChannelOverridesDefaultSmarthost",
			channels: Channels{
				{
					Name: "email-receiver",
					Type: "email",
					Data: `{"name":"email-receiver","email_configs":[{"to":"test@example.com","from":"legacy@example.com","smarthost":"localhost:25"}]}`,
				},
			},
			expectedRoutes: []map[string]any{{"receiver": "email-receiver", "continue": true, "matchers": []any{"ruleId=~\"-1\""}}},
			expectedReceivers: []map[string]any{
		...
		{
			name: "EmailChannelKeepsCustomSmarthost",
			channels: Channels{
				{
					Name: "email-receiver",
					Type: "email",
					Data: `{"name":"email-receiver","email_configs":[{"to":"test@example.com","smarthost":"smtp.custom.com:2525"}]}`,
				},
			},
			expectedRoutes: []map[string]any{{"receiver": "email-receiver", "continue": true, "matchers": []any{"ruleId=~\"-1\""}}},
			expectedReceivers: []map[string]any{
				{"name": "default-receiver"},
				{
					...
					"email_configs": []any{map[string]any{
						...
						"from":          "alerts@example.com",
						"hello":         "localhost",
						"smarthost":     "smtp.custom.com:2525",
						"require_tls":   true,
						"html":          "{{ template \"email.default.html\" . }}",
						"tls_config":    map[string]any{"insecure_skip_verify": false},
					}},
