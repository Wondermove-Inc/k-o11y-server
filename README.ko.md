...
**K-O11y Server — 백엔드. ServiceMap, S3 Tiering, SSO를 지원하는 SigNoz 기반 관측성 플랫폼.**
...
[SigNoz](https://signoz.io/) · [ClickHouse](https://clickhouse.com/) · [OpenTelemetry](https://opentelemetry.io/) 에코시스템 기반
...
K-O11y Server는 두 개의 패키지로 구성된 **모노레포 백엔드**입니다. Go 기반 `core` API(ServiceMap 배치 프로세서, S3 Tiering API)와 SigNoz 포크(`packages/signoz`)가 React 프론트엔드와 Query Service를 담당하며, 공통 ClickHouse 클러스터를 공유합니다.
...
flowchart TB
    ...
    subgraph Server["K-O11y Server (본 레포)"]
        ...
        subgraph SigNozPkg["packages/signoz (SigNoz 포크)"]
...
3. **조회(Query)** — `packages/signoz` Query Service가 SigNoz UI(메트릭/로그/트레이스)에 데이터를 제공하고 SSO + S3 티어링 설정을 관리
4. **시각화(Visualize)** — Frontend가 Core API(ServiceMap)와 SigNoz Query Service(표준 관측성 뷰)를 동시에 소비
...
| [`packages/signoz`](packages/signoz) | SigNoz 포크 (React UI + Query Service) | React 18 + Go + Webpack 5 |
...
│   │   └── Makefile
│   │
│   └── signoz/                      # SigNoz 포크 (UI + Query Service)
...
| [k-o11y-otel-gateway](https://github.com/Wondermove-Inc/k-o11y-otel-gateway) | License Guard를 갖춘 SigNoz OTel Collector |
